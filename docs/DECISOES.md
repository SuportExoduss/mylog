...
## D59 — White label muda a marca, e só a marca

O roadmap 7 pedia personalização por empresa com co-branding obrigatório, sobre
tokens semânticos, com contraste validado e prévia antes de publicar. Ao
implementar, três perguntas precisaram de resposta, e nenhuma era óbvia.

### 1. Quais tokens entram

A resposta curta é: **a família da marca, e nada mais** — `--marca`,
`--marca-forte`, `--marca-tinta`, `--marca-contraste`. Quatro, por tema.

O que fica de fora, e por quê: **as cores de estado**. Verde, amarelo e
vermelho não são identidade neste sistema — são vocabulário. Vermelho quer
dizer *ocorrência crítica, veículo bloqueado* em toda tela, todo relatório
impresso e todo dossiê de sinistro. Uma empresa que pintasse `crítico` de verde
não estaria mudando a aparência: estaria mudando **o que a tela afirma**, e
quem lê a tela decide se um caminhão sai ou não.

A arquitetura já dizia que branding não toca em regra. Cor de estado é regra
vestida de cor, e por isso `COR_DE_ESTADO` está declarada em
`compartilhado/marca.js` — não como validação a mais, mas como lista que o
código consulta para explicar a recusa em português.

### 2. Onde o contraste é conferido

Nos dois lados, com o **mesmo arquivo**. `compartilhado/contraste.js` implementa
a razão da WCAG 2.1; `compartilhado/marca.js` diz quais pares importam e qual é
o piso de cada um. O painel avisa enquanto a pessoa escolhe; o servidor recusa
com `400`. Duas implementações divergiriam no dia seguinte — e a divergência
seria "a tela disse que passava".

São três pares, e cada um existe por um motivo diferente:

| Par | Piso | Por quê |
|---|---|---|
| `marca-contraste` sobre `marca` | 4.5:1 | O texto **dentro** do botão principal |
| `marca` sobre o fundo da tela | 3:1 | Componente de interface: a borda do botão, o item ativo, o anel de foco |
| Texto sobre `marca-tinta` | 4.5:1 | A tinta é **fundo** de selo e de linha selecionada |

Contraste bom dentro do botão não basta: uma marca quase-branca passa lá e
**some** no fundo da tela. São dois pares porque são dois problemas.

O arredondamento é **para baixo**: 4.49 não pode virar "4.5, passa".

### 3. O padrão é o tema MyLog — e isso precisou de teste

A primeira versão dos padrões foi escrita de memória e errou dois:
`marca-contraste` no escuro (branco, quando o tema usa quase-preto) e
`marca-forte` no escuro (mais escura, quando o tema **clareia** — no escuro o
movimento é afastar-se do fundo, e o fundo é escuro).

O primeiro errado reprovava o próprio MyLog na conferência: branco sobre
`#6f8fd6` dá 3.1:1. A regra seria impossível de cumprir, e a primeira coisa que
a tela faria era recusar a si mesma.

Uma lista copiada à mão apodrece. O teste lê `web/css/estilo.css`, resolve a
camada de primitivas e compara com os padrões declarados.

### Como a marca chega

Dentro do **mesmo pacote que diz quem a pessoa é** — `/api/auth/login`,
`/api/auth/eu` e `/api/app/inicio`. Nunca numa chamada separada: assim não
existe instante em que a tela está logada mostrando a marca de uma empresa e os
dados de outra. Sair devolve a marca do MyLog, porque a tela de login não é de
empresa nenhuma — e deixar a marca da última ali diria, para quem pega o
aparelho depois, de quem ele estava.

### Como ela é pintada

Por **folha de estilo**, e não por `style.setProperty` no elemento raiz. A
primeira versão fazia isso e estava errada de dois jeitos.

O `DESIGN.md` proíbe estilo embutido no JS, e há varredura cobrando. Mas o
motivo que importa é outro: **estilo em linha não tem tema**. Um conjunto único
de tokens sobreviveria à troca de claro para escuro, e a cor do tema anterior
ficaria por cima do tema que acabou de entrar — inclusive quando o tema muda
sozinho, porque o sistema mudou. A folha espelha a cascata que o `estilo.css`
já usa (claro, media query, `[data-tema]`), e o navegador resolve.

A prévia é uma **segunda folha**, depois da primeira: enquanto existe, vence a
marca publicada; some, e a publicada volta sozinha. Sem estado para desfazer
errado.

### A prévia precisa aparecer

Editar as cores do tema claro com o painel no escuro não mudava **nada** na
tela — e quem faz isso conclui, com razão, que a função está quebrada. Mexer
nas cores de um tema agora leva o painel para ele, mexendo no `data-tema` do
documento e **não** no `localStorage`: a preferência da pessoa volta inteira ao
sair da tela.

### O que a ausência garante

Não há campo para desligar o co-branding, e é por isso que não há caminho. Um
`esconder_mylog` no corpo da requisição não é ignorado por uma validação que
alguém pode afrouxar: ele não existe como token, e o `PUT` só copia chaves da
...
válida*. Por isso o nome do arquivo é sorteado, nunca fixo: grava o arquivo
**novo**, aponta o banco para ele, e só então apaga o antigo. Falhando a
gravação, o banco segue apontando para a logo que funciona; falhando o banco,
sobra um arquivo órfão — que é o lado barato de errar.

O tipo sai dos **bytes** ([D38](#d38--quem-diz-o-que-o-arquivo-é-são-os-bytes)),
e SVG não entra: SVG carrega script, e a logo é servida dentro do painel.

### Um teste que não podia falhar

O primeiro teste de isolamento da logo dava logo só para a empresa A. Arrancando
a conferência de tenant da rota, ela passava a servir a marca de **quem pediu** —
e a B, sem logo, tomava `404` do mesmo jeito. Verde com a guarda arrancada: o
`404` vinha pelo motivo errado.

Com logo nos dois lados, a falta da guarda vira o que ela é de verdade: um `200`
devolvendo a imagem errada, num endereço que diz outra empresa.
